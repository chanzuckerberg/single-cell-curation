...
def _validate_human_readable_string(s):
    ...
    return (
        isinstance(s, str)
        and (not re.match(r"[A-Z]\w+:\d+", s))
        and (not re.match(r"ENS[GT]\d+$", s))
    )
...
def _validate_column(column, column_name, df_name, schema_def):
    ...
    if schema_def.get("type") in ("curie", "suffixed curie"):
        validation_func = (
            ...
            else _validate_suffixed_curie
        )
        non_valid_curies = [
            v for v in column if not validation_func(v, schema_def.get("prefixes"))
        ]
        if non_valid_curies:
            errors.append(
            ...
            if "prefixes" in schema_def:
                errors[
                    -1
                ] += f" Values must be curies from one of these ontologies {schema_def['prefixes']}."
        ...
        if key not in dict_:
            ...
            if schema_def["keys"][key]["type"] == "stringified list of dicts":
                ...
                if not _validate_curie(dict_[key], schema_def["keys"][key]["prefixes"]):
                    errors.append(
                        f"Key {key} in {dict_name} contains invalid ontology value."
                    )
            if (
                "nullable" in schema_def["keys"][key]
                and not schema_def["keys"][key]["nullable"]
            ):
...
def _validate_dataframe(df, df_name, schema_def):
    ...
    for column in schema_def.get("columns", []):
        if column not in df.columns:
            ...
            errors.extend(
                _validate_column(
                    df[column], column, df_name, schema_def["columns"][column]
                )
            )
...
def validate_adata(adata, shallow):
    ...
    if (
        "version" not in adata.uns_keys()
        or "corpora_schema_version" not in adata.uns["version"]
    ):
