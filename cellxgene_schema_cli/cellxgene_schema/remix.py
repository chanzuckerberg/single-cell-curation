        ...
        if isinstance(field_value, dict):
            ...
            for value in adata.obs[source_column].unique():
                if value not in column_map:
                    logging.warning(
                        f'Value {value} in adata.obs["{source_column}"] not in translation dict'
                    )
...
def merge_df(df, domain, index, columns):
    ...
    if domain == "raw":
        ...
        merged_df = pd.DataFrame(
            np.expm1(to_merge, dtype=np.float128), index=index, columns=columns
        ).sum(axis=1, level=0, skipna=False)
        merged_df = pd.DataFrame(
            np.log1p(merged_df.to_numpy()),
            index=merged_df.index,
            columns=merged_df.columns,
        )
    elif domain == "sqrt":
        merged_df = pd.DataFrame(np.square(to_merge), index=index, columns=columns).sum(
            axis=1, level=0, skipna=False
        )
        merged_df = pd.DataFrame(
            np.sqrt(merged_df.to_numpy()),
            index=merged_df.index,
            columns=merged_df.columns,
        )
...
def apply_schema(source_h5ad, remix_config, output_filename):
    ...
    if (
        ...
        try:
            ...
            logging.warning(
                f"Stripping version information out of AnnData because schema "
