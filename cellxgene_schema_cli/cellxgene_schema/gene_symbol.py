...
def get_upgraded_var_index(var, hgnc_path=None):
    ...
    if not hgnc_path:
        hgnc_path = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "hgnc_complete_set.txt.gz"
        )
...
class HGNCSymbolChecker:
    ...
    def __init__(
        self, approved_symbols, withdrawn_symbols, ambiguous_symbols, symbol_map
    ):
    ...
    def from_hgnc_records(cls, hgnc_dataset_path):
        ...
        hgnc_records = pd.read_csv(
            hgnc_dataset_path, sep="\t", header=0, low_memory=False
        ).to_dict("records")
        ...
        for record in hgnc_records:
            if record["status"] == "Approved":
                ...
                for symbol in alias_and_previous_symbols(record):
                    ...
                    if (
                        symbol in alias_previous_to_approved
                        and alias_previous_to_approved[symbol] != approved_symbol
                    ):
...
def main():
    ...
    parser.add_argument(
        "hgnc_dataset",
        help="HGNC dataset tsv, available from www.genenames.org/download/statistics-and-files/",
    )
