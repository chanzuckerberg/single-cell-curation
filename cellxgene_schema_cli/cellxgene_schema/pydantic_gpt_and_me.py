...
def is_matrix_sparse(matrix: np.ndarray, sparse_threshold):
    ...
    # TODO consolidate with the validators sparity check.
    """
...
class AnnData(BaseModel):
    X: Union[np.ndarray, sparse.spmatrix, SparseDataset]

    @field_validator("X")
    @classmethod
    def check_sparse_threshold(cls, v):
        if is_matrix_sparse(v, 75):
            raise ValueError("sparse_threshold must be between 0 and 100")
        return v


def test_X():
    with pytest.raises(ValidationError):
        AnnData(X=h5ad_valid.X)
