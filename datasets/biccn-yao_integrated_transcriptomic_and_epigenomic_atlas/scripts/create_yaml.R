...
main <- function(cmdArgs=commandArgs(T)) {
    ...
    yaml_final <- make_yaml_list(Title=Title, assay=assay, male=male, female=female)
    ...
    # Appending to yaml
    yaml_final$obs <- c(yaml_final$obs, list(cell_type_ontology_term_id = list(temp_class_label = cell_type_yaml)))
...
make_yaml_list <- function(Title, assay, male, female) {
    
    x <- list(
         #fixup_gene_symbols = list(X = "log1p", raw.X = "raw"),
         obs = list(assay_ontology_term_id = assay,
         ...
         uns = list(version = list(corpora_schema_version="1.1.0", corpora_encoding_version="0.1.0"),
                    ...
                    publication_doi = "https://doi.org/10.1101/2020.02.29.970558", 
                    title = paste("An integrated transcriptomic and epigenomic atlas of mouse primary motor cortex cell types:", Title)
                    )
         )
    
    names(x$obs$sex$sex) <- c(male, female)
