...
main <- function(cmdArgs=commandArgs(T)) {
    ...
    title_suffix <- cmdArgs[2]
    out_file <- cmdArgs[3]
    
    #cell_type_file <- '../data/misc/sample.combined_exc_4_species_integration.h5ad_ontology_lookup_cell_type.tsv'
    
    # Create backbone of the yml with manual values
    yaml_final <- make_yaml_list(title_suffix)
...
make_yaml_list <- function(title_suffix) {
